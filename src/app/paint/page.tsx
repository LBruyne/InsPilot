...
import {
    ADD_SCHEME,
    PaintProvider, SWITCH_STAGE,
    UPDATE_CURRENT_SCHEME, UPDATE_DESIGN_TASK,
    UPDATE_SELECTED_SCHEMES,
    usePaintContext
} from "@/app/paint/provider";
import React, {ReactNode, useEffect, useRef, useState} from "react";
import {convergenceMinimizeSchemeNumber, Stage} from "@/app/paint/config";
import {RouteConfig} from "@/constant/route-config";
import {LeftSide, LeftSideHandler} from "@/app/paint/(left-side)";
...
async function getRapidDivergenceStimulus(task: string, num: number) {
...
export default function Page() {
    ...
    const handleClickFinishInputButton = async () => {
        if (currentDesignTaskInput && currentDesignTaskInput.length > 0) {
            ...
            // 获取新的刺激
            await getRapidDivergenceStimulus()
        }
    };
